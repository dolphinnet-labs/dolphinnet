...
Issues:
[monorepo](https://github.com/dolphinnet-labs/dolphinnet/issues?q=is%3Aissue%20state%3Aopen%20label%3AA-dn-node)

Pull requests:
[monorepo](https://github.com/dolphinnet-labs/dolphinnet/pulls?q=is%3Aopen+is%3Apr+label%3AA-dn-node)
...
The dn-node implements the [rollup-node spec].
...
# - RPC port, to serve RPC of the dn-node
#
# Other:
...
#
# Tip: every CLI flag has an env-var equivalent (run `dn-node --help` for more information)
./bin/dn-node \
  --network=op-sepolia \
...
See `dn-node` docker-bake target.
...
The dn-node **builds**, **relays** and **verifies** the canonical chain of blocks.

The dn-node does not store critical data:
the dn-node can recover from any existing L2 chain pre-state
that is sufficiently synced such that available input data can complete the sync.

The dn-node **builds** blocks:
...
The dn-node implements the three core product features as following:
...
The dn-node is changing in two ways:
...
- Parallel derivation processes: [Issue 10864](https://github.com/dolphinnet-labs/dolphinnet/issues/10864)
- Event tests: [Issue 13163](https://github.com/dolphinnet-labs/dolphinnet/issues/13163)
- Improving P2P sync: [Issue 11779](https://github.com/dolphinnet-labs/dolphinnet/issues/11779)
...
Asynchronous verification entails that the dn-node reorgs away a block
if and when the block is determined to be invalid.

The [dn-supervisor] specializes in this dependency verification work.

The dn-node encapsulates all the single-chain concerns:
it prepares the local safety data-points (DA confirmation and block contents) for the dn-supervisor.

The dn-supervisor then verifies the cross-chain safety, and promotes the block safety level accordingly,
which the dn-node then follows.

See [Interop specs] and [Interop design-docs] for more information about interoperability.

[dn-supervisor]: ../dn-supervisor/README.md
...
- Encapsulate the state-transition:
  ...
  - Ensure code-sharing with action-tests and dn-program.
...
This is a brief overview of what might fail, and how the dn-node responds.

### L1 downtime

When the L1 data-source is temporarily unavailable the dn-node `safe`/`finalized` progression halts.
...
The dn-node will continue to process the happy-path in the mean time,
...
When this happens, the dn-node assumes the local view is wrong, and resets itself to follow that of the remote node,
dropping any non-canonical blocks in the process.

### No L1 finality

When L1 does not finalize for an extended period of time,
the dn-node is also unable to finalize the L2 chain for the same time.
...
The dn-node may pick back up the latest `unsafe` blocks after recovering its P2P connectivity,
...
the dn-node dynamically determines what L1 data is required to continue, based on the syncing state of execution-engine.
If the sync-state is far behind, the dn-node may need archived blob data to sync from the original L1 inputs.
...
- Unit tests: encapsulated functionality, fuzz tests, etc. in the dn-node Go packages.
...
- `op-e2e` system tests: in-process Go testing, focused on the offchain aspects of the dn-node,

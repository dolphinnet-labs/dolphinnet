...
// There are two kinds of sync mode that the dn-node does:
//  1. In consensus-layer (CL) sync, the dn-node fully drives the execution client and imports unsafe blocks &
//     fetches unsafe blocks that it has missed.
//  2. In execution-layer (EL) sync, the dn-node tells the execution client to sync towards the tip of the chain.
...
type Config struct {
	...
	// This defers the L1-origin verification, and is recommended to use in when utilizing --syncmode=execution-layer on dn-node and --syncmode=snap on op-geth

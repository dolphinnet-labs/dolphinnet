...
import (
	...
	"github.com/dolphinnet-labs/dolphinnet/dn-node/rollup/clsync"
...
type SyncDeriver struct {
	...
	// When in interop, and managed by an dn-supervisor,
	// the node performs a reset based on the instructions of the dn-supervisor.
	ManagedMode bool
}
...
func (s *SyncDeriver) onResetEvent(x rollup.ResetEvent) {
	if s.ManagedMode {
		s.Log.Warn("Encountered reset in Managed Mode, waiting for dn-supervisor", "err", x.Err)
...
// Warning: this sequence will be broken apart as outlined in dn-node derivers design doc.

...
Issues: [monorepo](https://github.com/dolphinnet-labs/dolphinnet/issues?q=is%3Aissue%20state%3Aopen%20label%3AA-dn-supervisor)

Pull requests: [monorepo](https://github.com/dolphinnet-labs/dolphinnet/pulls?q=is%3Aopen+is%3Apr+label%3AA-dn-supervisor)

User docs:
- [dn-supervisor](https://docs.optimism.io/stack/interop/dn-supervisor)

Specs:
- [interop specs]

`dn-supervisor` is a service to monitor chains, and quickly determine
cross-chain message safety, for native interoperability.
The `dn-supervisor` functions as a [superchain backend], implementing the [interop specs].
...
```bash
# from dn-supervisor dir:
make dn-supervisor
./bin/dn-supervisor --help
```
...
See `dn-supervisor` docker-bake target.
...
The dn-supervisor tracks these dependencies, to maintain a global view of cross-chain message safety.
...
dn-nodes, or any compatible consensus-layer L2 node can interact with dn-supervisor in two modes:
...
The Supervisor subscribes to events from the dn-node in order to react appropriately.
In turn, dn-supervisor sends signals to the dn-node to manage its derivation pipeline or maintain databases:

| dn-node event | dn-supervisor control |
|-------|---------|
...
Additionally, the supervisor sends control signals to the dn-node triggered by *database updates* in order to inform the node of cross-safety levels:
| database event | dn-supervisor control |
|-------|---------|
...
In standard mode, an `dn-node` continues to handle derivation and L1 discovery as it would without a supervisor.
...
participant super as dn-supervisor
participant node as dn-node (managed)
...
The dn-supervisor maintains a few databases:
...
Additionally, the dn-supervisor tracks `cross unsafe` in memory, not persisting it to a database:
...
flowchart TD
    user-->opnode
    user-->opgeth
    opnode[dn-node]==block checks==>frontend[frontend RPC]
...
The `dn-supervisor` implementation optimizes safe determination of cross-chain message safety,
...
The `dn-supervisor` is actively changing.
...
Further background on the design-choices of dn-supervisor can be found in the
...
- A read-only subset of the API is served, sufficient for nodes to stay in sync, assuming a healthy dn-supervisor.

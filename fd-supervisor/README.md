...
Issues: [monorepo](https://github.com/flexdeal-chain/fd-chain/issues?q=is%3Aissue%20state%3Aopen%20label%3AA-fd-supervisor)

Pull requests: [monorepo](https://github.com/flexdeal-chain/fd-chain/pulls?q=is%3Aopen+is%3Apr+label%3AA-fd-supervisor)

User docs:
- [fd-supervisor](https://docs.optimism.io/stack/interop/fd-supervisor)

Specs:
- [interop specs]

`fd-supervisor` is a service to monitor chains, and quickly determine
cross-chain message safety, for native interoperability.
The `fd-supervisor` functions as a [superchain backend], implementing the [interop specs].
...
```bash
# from fd-supervisor dir:
make fd-supervisor
./bin/fd-supervisor --help
```
...
See `fd-supervisor` docker-bake target.
...
The fd-supervisor tracks these dependencies, to maintain a global view of cross-chain message safety.
...
fd-nodes, or any compatible consensus-layer L2 node can interact with fd-supervisor in two modes:
...
The Supervisor subscribes to events from the fd-node in order to react appropriately.
In turn, fd-supervisor sends signals to the fd-node to manage its derivation pipeline or maintain databases:

| fd-node event | fd-supervisor control |
|-------|---------|
...
Additionally, the supervisor sends control signals to the fd-node triggered by *database updates* in order to inform the node of cross-safety levels:
| database event | fd-supervisor control |
|-------|---------|
...
In standard mode, an `fd-node` continues to handle derivation and L1 discovery as it would without a supervisor.
...
participant super as fd-supervisor
participant node as fd-node (managed)
...
The fd-supervisor maintains a few databases:
...
Additionally, the fd-supervisor tracks `cross unsafe` in memory, not persisting it to a database:
...
flowchart TD
    user-->opnode
    user-->opgeth
    opnode[fd-node]==block checks==>frontend[frontend RPC]
...
The `fd-supervisor` implementation optimizes safe determination of cross-chain message safety,
...
The `fd-supervisor` is actively changing.
...
Further background on the design-choices of fd-supervisor can be found in the
...
- A read-only subset of the API is served, sufficient for nodes to stay in sync, assuming a healthy fd-supervisor.

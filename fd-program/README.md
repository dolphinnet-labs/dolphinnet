...
To build fd-program, from within the `fd-program` directory run:

```shell
make fd-program
```

This resulting executable will be in `./bin/fd-program`

## Testing

To run fd-program unit tests, from within the `fd-program` directory run:
...
From within the `fd-program` directory, options can be reviewed with:
...
The absolute pre-state of the fd-program can be generated by executing the makefile
...
[./bin/](./bin/):
- [`fd-program`](./bin/fd-program)
- [`fd-program-client.elf`](./bin/fd-program-client.elf)
- [`fd-program-client64.elf`](./bin/fd-program-client64.elf)
...
[ctb]: ../packages/contracts-fd-chain/
